...
export default function ZoneTransition() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(streaksRef.current?.children ?? [],
      ...
      { scaleX: 1, opacity: 0, duration: 0.25, stagger: 0.04, ease: 'power4.out' },
      '-=0.3'
    )
    ...
    tl.fromTo([numberGhostR.current, numberGhostC.current],
      { scale: 4, opacity: 0 },
      { scale: 1, opacity: 0.5, duration: 0.4, ease: 'expo.out', stagger: 0.04 },
      '-=0.1'
    )
    ...
    .to(overlayRef.current,
      { opacity: 0, duration: 0.2, onComplete: resumeFromTransition }
    )
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: `radial-gradient(ellipse at center, ${color}30 0%, transparent 60%)` }}
      />

      {/* Top panel (black slab) */}
      <div ref={panelTopRef} className="absolute top-0 w-full"
        style={{ height: '50%', background: `linear-gradient(to bottom, #000 80%, ${color}22)`, transformOrigin: 'top center' }}
      />
      {/* Bottom panel */}
      <div ref={panelBotRef} className="absolute bottom-0 w-full"
        style={{ height: '50%', background: `linear-gradient(to top, #000 80%, ${color}22)`, transformOrigin: 'bottom center' }}
      />

      {/* Horizontal scan line */}
      <div ref={scanRef} className="absolute w-full pointer-events-none"
        style={{ height: '3px', top: '0%', background: `linear-gradient(to right, transparent, ${color}, transparent)`,
          boxShadow: `0 0 20px 4px ${color}88`, opacity: 0 }}
      />
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-8">
        ...
        <div className="relative mb-2" style={{ lineHeight: 1 }}>
          ...
          <div ref={numberGhostR} className="absolute inset-0 flex items-center justify-center font-mono font-black select-none pointer-events-none"
            style={{ fontSize: 'clamp(5rem, 18vw, 9rem)', color: '#ff2020', opacity: 0,
              transform: 'translate(-6px, 4px)', filter: 'blur(1px)' }}>
            {zone}
          </div>
          {/* Cyan ghost */}
          <div ref={numberGhostC} className="absolute inset-0 flex items-center justify-center font-mono font-black select-none pointer-events-none"
            style={{ fontSize: 'clamp(5rem, 18vw, 9rem)', color: '#00f5ff', opacity: 0,
              transform: 'translate(6px, -4px)', filter: 'blur(1px)' }}>
            {zone}
          </div>
          {/* Main number */}
          <div ref={numberRef} className="font-mono font-black"
            style={{ fontSize: 'clamp(5rem, 18vw, 9rem)', color,
              textShadow: `0 0 60px ${color}cc, 0 0 120px ${color}44`, opacity: 0 }}>
            {zone}
          </div>
        </div>

        <h2 ref={nameRef} className="font-mono tracking-[0.35em] text-white mb-4"
          style={{ fontSize: 'clamp(1rem, 3vw, 1.6rem)', opacity: 0 }}>
          {zoneData.name.toUpperCase()}
        </h2>

        <p ref={taglineRef} className="font-mono text-xs tracking-widest max-w-xs"
      ...
      <div className="absolute bottom-8 w-56">
        ...
        <p ref={progressLabelRef} className="text-center text-[9px] tracking-widest text-white/30 font-mono mt-2"
          style={{ opacity: 0 }}>
      ...
      {/* Corner brackets */}
      <div className="absolute top-4 left-4 w-6 h-6 border-t-2 border-l-2" style={{ borderColor: color }} />
      <div className="absolute top-4 right-4 w-6 h-6 border-t-2 border-r-2" style={{ borderColor: color }} />
      <div className="absolute bottom-4 left-4 w-6 h-6 border-b-2 border-l-2" style={{ borderColor: color }} />
      <div className="absolute bottom-4 right-4 w-6 h-6 border-b-2 border-r-2" style={{ borderColor: color }} />
    </div>
  )
}
